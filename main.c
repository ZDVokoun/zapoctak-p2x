...
void b64_init(struct Base2_64Int *bn, size_t initial_cap) {
  bn->limbs =
      calloc(initial_cap, sizeof(uint64_t)); // Casting void* not needed in C
  bn->len = 0;
...
void b64_mul(struct Base2_64Int *bn, uint64_t multiplier, uint64_t addend) {
  ...
  for (size_t i = 0; i < bn->capacity; i++) {
    if (carry == 0 && bn->limbs[i] == 0)
      break;
    uint128_t product = (uint128_t)bn->limbs[i] * multiplier + carry;
  ...
  if (carry > 0) {
    if (bn->len >= bn->capacity) {
      ...
      memset(bn->limbs + bn->capacity, 0,
             (bn->capacity - bn->capacity) * sizeof(uint64_t));
    }
...
void base2_64_to_residue(const struct Base2_64Int *bn, size_t minimumSz,
  ...
  * and their properties described in "The Art of Computer Programming, Volume 2:
  * Seminumerical Algorithms" by Donald E. Knuth, Section 4.3.2., Modular Arithmetic
  */
  if (bn->len * 64 > minimumSz) minimumSz = bn->len * 64;
    ...
    for (size_t j = 0; j < bn->len*64; j += modulusPow) {
      ...
      if (current_bit + modulusPow > 64) {
        ...
        if (bn->len > limb_index + 1) {
          upper_part = bn->limbs[limb_index + 1] & (modulus >> (64 - current_bit));
        }
...
void residue_mul(const struct ResidueInt *a, const struct ResidueInt *b) {
  ...
  for (size_t i = 0; i < a->len; i++) {
    uint64_t modulus = (1ULL << moduli64[i]) - 1;
    uint128_t product = (__uint128_t)a->residues[i] * b->residues[i];
...
void residue_to_base2_64(const struct ResidueInt *res, struct Base2_64Int *bn) {
  ...
  // Representation of the integer in mixed radix system with bases being the moduli
  // It's going to be of a form u = v_0 + v_1 * m_0 + v_2 * m_0 * m_1 + ... + v_n * m_0 * m_1 * ... * m_(n-1)
  uint64_t v[res->len];
  for (size_t i = 0; i < res->len; i++) {
    v[i] = res->residues[i];
  }
  ...
  for (size_t i = 0; i < res->len; i++) {
    for (size_t j = 0; j < i; j++) {
      uint64_t prev_modulus = precomputed_moduli[j];
      v[i] = (v[i] - v[j] + prev_modulus) % prev_modulus;
      ...
      while (exp != 1) {
        c += 1 << exp;
        exp = (exp + d) % moduli64[j];
      }

      v[i] = (v[i] * c) % prev_modulus;
    }
  }
...
uint64_t fast_div128_64(uint64_t high, uint64_t low, uint64_t divisor, uint64_t *rem) {
    ...
    __asm__ (
        "divq %[v]"
        : "=a"(quotient), "=d"(*rem) // Outputs: a=RAX (quotient), d=RDX (remainder)
...
void base2_64_divmod(struct Base2_64Int *bn, uint64_t divisor, uint64_t *remainder) {
    uint64_t rem = 0;
...
void base2_64_print_decimal(const struct Base2_64Int *bn) {
    ...
    *--p = '\0';
    ...
    while (temp.len > 0) {
        ...
        *--p = '0' + (char)rem;
        ...
        while (temp.len > 0 && temp.limbs[temp.len - 1] == 0) {
            temp.len--;
        }
    }
...
int main() {
  ...
  base2_64_to_residue(&bn, 0, &res1);
  base2_64_to_residue(&bn, 0, &res2);
